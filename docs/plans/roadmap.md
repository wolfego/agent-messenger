...
Everything below was delivered during the initial build sprint (2026-04-06 to 2026-04-07). Listed for context — these are not roadmap items.

- MCP server with 9 tools: `send_message`, `check_inbox`, `reply`, `get_thread`, `list_conversations`, `mark_read`, `whoami`, `set_channel`, `set_identity`
...
| 7.1.4 | Color-coded terminal output (unread = bold, urgent = red) | P3 |
...
| 7.2.2 | Filter by agent, channel, time range | P3 |
...
| Task | Description | Priority |
|------|-------------|----------|
| 8.2.1 | Add optional `task_id` field to `send_message` and `reply` — attaches `refs:bd-XX` label | P2 |
| 8.2.2 | When replying with a task reference, auto-update task notes with message summary | P3 |
| 8.2.3 | `show_task` includes linked message thread if one exists | P3 |

### Epic 8.3: Task Shortcuts

| Task | Description | Priority |
|------|-------------|----------|
| 8.3.1 | Cursor shortcuts: `#ct` create task, `#lt` list tasks, `#st` show task, `#rt` ready tasks | P2 |
| 8.3.2 | CC skills: `/ct`, `/lt`, `/st`, `/rt` | P2 |
...
### Epic 10.1: Presence & Discovery

| Task | Description | Priority |
|------|-------------|----------|
| 10.1.1 | Implement heartbeat: each agent writes an ephemeral presence bead on startup and periodically | P3 |
| 10.1.2 | `list_agents` MCP tool: shows which agents are currently active (based on recent heartbeats) | P3 |
